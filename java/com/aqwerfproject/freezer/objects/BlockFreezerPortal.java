...
import com.aqwerfproject.freezer.world.FreezerTeleporter;
...
public class BlockFreezerPortal extends BlockBreakable {
	...
	public BlockFreezerPortal(String name, Material materialIn) {
			...
			if (!entityIn.world.isRemote) {
				if (entityIn instanceof EntityPlayerMP) {
					...
					if (player1.timeUntilPortal > 0) {
						...
						player1.timeUntilPortal = 10;
						if (player1.dimension == 0) {
							mcServer.getPlayerList().transferPlayerToDimension(player1, FreezerWorldGen.FREEZER_ID,
									new FreezerTeleporter(mcServer.getWorld(FreezerWorldGen.FREEZER_ID)));

						} else if (player1.dimension == FreezerWorldGen.FREEZER_ID) {
							mcServer.getPlayerList().transferPlayerToDimension(player1, 0,
									new FreezerTeleporter(mcServer.getWorld(0)));
						}
					}
				}
			}
	...
	public boolean trySpawnPortal(World worldIn, BlockPos pos) {
		BlockFreezerPortal.Size blockportal$size = new BlockFreezerPortal.Size(worldIn, pos, EnumFacing.Axis.X);
		// if (blockportal$size.isValid() && blockportal$size.portalBlockCount == 0 &&
		// !net.minecraftforge.event.ForgeEventFactory.onTrySpawnPortal(worldIn, pos,
		// blockportal$size))
		if (blockportal$size.isValid() && blockportal$size.portalBlockCount == 0) {
			blockportal$size.placePortalBlocks();
			return true;
		} else {
			BlockFreezerPortal.Size blockportal$size1 = new BlockFreezerPortal.Size(worldIn, pos, EnumFacing.Axis.Z);

			if (blockportal$size1.isValid() && blockportal$size1.portalBlockCount == 0) {
				// && !net.minecraftforge.event.ForgeEventFactory.onTrySpawnPortal(worldIn,
				// pos,blockportal$size1)) {
	...
	public BlockPattern.PatternHelper createPatternHelper(World worldIn, BlockPos p_181089_2_) {
		...
		BlockFreezerPortal.Size blockportal$size = new BlockFreezerPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.X);
		LoadingCache<BlockPos, BlockWorldState> loadingcache = BlockPattern.createLoadingCache(worldIn, true);

		if (!blockportal$size.isValid()) {
			enumfacing$axis = EnumFacing.Axis.X;
			blockportal$size = new BlockFreezerPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.Z);
		}
	...
	public static class Size {
		...
		protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
			...
			for (i = 0; i < 22; ++i) {
				...
				if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
						|| this.world.getBlockState(blockpos.down()).getBlock() != BlockInit.BLOCK_BLUEICE) {
					break;
				}
			}

			Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
			return block == BlockInit.BLOCK_BLUEICE ? i : 0;
		}
		...
		protected int calculatePortalHeight() {
			...
			for (this.height = 0; this.height < 21; ++this.height) {
				for (int i = 0; i < this.width; ++i) {
					...
					if (block == BlockInit.BLOCK_FREEZER_PORTAL) {
					...
					if (i == 0) {
						...
						if (block != BlockInit.BLOCK_BLUEICE) {
							break label56;
						}
			...
			for (int j = 0; j < this.width; ++j) {
				if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height))
						.getBlock() != BlockInit.BLOCK_BLUEICE) {
					this.height = 0;
					break;
				}
			}
		...
		protected boolean isEmptyBlock(Block blockIn) {
			return blockIn == Blocks.AIR || blockIn == BlockInit.BLOCK_FREEZER_PORTAL;
		}
		...
		public void placePortalBlocks() {
			for (int i = 0; i < this.width; ++i) {
				...
				for (int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j), BlockInit.BLOCK_FREEZER_PORTAL.getDefaultState()
							.withProperty(BlockFreezerPortal.AXIS, this.axis), 2);
				}
			}
		}
	}
